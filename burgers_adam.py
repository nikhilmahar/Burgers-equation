...
# def normalize(x):
#     print("x",x)
#     xnorm=((x-torch.tensor([-1]))/(torch.tensor([1])-torch.tensor([-1])))
...
def initial_loss(xi):
    ...
    # loss3= torch.mean((output_ic+ torch.sin(math.pi*xi[:,0]))**2)
    return loss3
...
scheduler = StepLR(adam, step_size=2000, gamma=0.1)
for epoch in range(epochs):
    ...
    # Evaluate the loss
      loss1 = beta*residue_loss(xt.float())
      loss2= gamma*boundary_loss(xtb1.float(),xtb2.float())
...
xtTEST = torch.tensor(np.stack([xTEST.flatten(), tTEST.flatten()], axis=-1))
with torch.no_grad():
    u = N(xtTEST.float())
...
for i, t_cs in enumerate(t_cross_sections):
    plt.subplot(gs[1, i])
    xt1 = torch.tensor(np.stack([x_flat,np.full(t_flat.shape, t_cs) ], axis=-1))
    with torch.no_grad():
